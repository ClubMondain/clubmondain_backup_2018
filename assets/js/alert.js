...
function Register()
{
	...
	var members_phone    = $("#members_phone_data").val();
				...
				if( members_password == ''){
					...
					return false;
						...
						if( members_passconf != members_password || members_passconf == '' )
						{
							...
							if( members_phone == ''){
								alert_message('Phone is Required','warning','btn-warning','Please Choose Your Password!');
					return false;
				}else{
...
function loginUserData()
{
	...
	$.ajax({
	type: "POST",
	url: 'https://www.clubmondain.com/Home/loginUser',
	dataType:'html',
	...
	if(result != "SUCCESS_LOGIN" && result != "ERRORLOGIN"){
...
function favourite_event_remove(e)
{
	var text = '';
	var res = e.split("_");
	$.ajax({
	...
	if(result == 'SUCCESS')
	{
	window.location.href = conurl+"Main/my-favourites";
	}
...
function favourite_store_remove(e)
{
	var text = '';
	var res = e.split("_");
	$.ajax({
...
function category_event_search(e)
{
	...
	$.ajax({
		url: "https://www.clubmondain.com/Home/category_event_find/",
		dataType: 'HTML',
		type: 'POST',
		data:{
			send_id:send_id,
