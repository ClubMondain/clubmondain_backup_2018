...
public function __construct()
{
	...
	/////ADD BY SUBHASIS FOR INVITE SPACE PUBLIC OR PRIVATE///
...
public function loginUser()
{
	...
	/*echo $this->session->userdata("energizer_url");
	exit;*/
	...
	$this->__data = $this->urs->loginUsers(USER,$this->__email,$this->__password);
			...
			else
			{
				...
				if(!empty($this->session->userdata("energizer_url"))){
					echo $this->session->userdata("energizer_url");
				}else{
					echo "SUCCESS_LOGIN";
				}
		...
		$get_membership_id = $this->session->userdata('membership_id');
			if(!empty($get_membership_id))
			{
				...
				if($get_details[0]['membership_type'] == 'FREE'){
					...
					if(!empty($this->session->userdata("energizer_url")))
					{
						redirect( $this->session->userdata("energizer_url"));
					}
					else
					{
						redirect('Home');
...
/////ADD BY SUBHASIS FOR INVITE SPACE PUBLIC OR PRIVATE///
$detailSection['all_invitespace'] 	= $this->energiserdata->getInviteSpaceDetails(INVITE_SPACE,1);
/////ADD BY SUBHASIS FOR INVITE SPACE PUBLIC OR PRIVATE///
$this->load->view("event",$detailSection);
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

/////ADD BY SUBHASIS FOR INVITE SPACE DETAILS START///

public function invite_space_details($space_id){	
		...
		$this->load->view("space_details",$msgsection);
}

/////ADD BY SUBHASIS FOR INVITE SPACE DETAILS END///


/////ADD BY SUBHASIS FOR ENERGIZER LISTING START///
public function energiser_listing($invite_space_id)
{
		...
		$this->load->view("listing_energiser",$msgsection);
}

/////ADD BY SUBHASIS FOR ENERGIZER LISTING END///





/////ADD BY SUBHASIS FOR ENERGIZER DETAILS START///

public function energiser_details($id='')
{
		...
		exit;	
			*/
		$msgsection['joined_users'] = $this->urs->getJoinUser(USER_DETAILS,JOIN_US_ENERGIZER,'trainer_class_id',$id);		
			
		$this->load->view('details_energiser',$msgsection);

	}

/////ADD BY SUBHASIS FOR ENERGIZER DETAILS END///


public function join_energizer($user_id,$energizer_id)
{
...
public function join_energizer_withpay($event_id='')
{
	if(!empty($event_id))
	{
		...
		if(!empty($event_id))
		{
			$this->session->set_userdata('energizer_id',$event_id);
			
			$all_data_only = $this->urs->getFullUserDetails(ENERGISER,$event_id,'trainer_class_id');		
			$returnURL = base_url().'paypal/energiser_payment_success'; //payment success url
			$cancelURL = base_url().'paypal/cancel'; //payment cancel url
			$notifyURL = base_url().'paypal/ipn'; //ipn url
...
///JOIN ENERGIZER WITH FREE /// 

public function join_energizer_withfree($user_id,$energizer_id)
{
		$energiser_seat = $this->energiserdata->getEnergiserSeatCount($energizer_id);
...
public function remove_energizer($user_id,$energizer_id)
	{
